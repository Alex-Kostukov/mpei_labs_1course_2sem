...
begin
  ...
  while (k<>nil) do
  begin
    ListBox.Items.Add(FloatToStr(k^.info));
    k := k^.next;
  end;
end;
...
begin
  ...
  while (k.head<>nil) do
  begin
    ListBox.Items.Add(FloatToStr(k.head^.info));
    k.head := k.head^.next;
  end;
end;
...
var n:tinfo;
begin
  n := strtoFloat(EditElemDeck.Text);
...
var n:tinfo;
begin
  n := strtoFloat(EditElemQueue.Text);
  AddQueue(qu,n);
...
var n:tinfo;
begin
  n := strtoFloat(EditElemStack.Text);
  AddStack(st,n);
...
begin
  if (strtofloat(EditElemDeck.Text)<-99.9) and (strtofloat(EditElemDeck.Text)>99.9)  then
    EditElemDeck.Text := ''
  else
  begin
...
begin
  if (strtofloat(EditElemQueue.Text)<-99.9) and (strtofloat(EditElemQueue.Text)>99.9)  then
    EditElemQueue.Text := ''
  else
  begin
...
begin
  if (strtofloat(EditElemStack.Text)<-99.9) and (strtofloat(EditElemStack.Text)>99.9)  then
    EditElemStack.Text := ''
  else
  begin
...
begin
Application.MessageBox('Из стека/очереди удалить эл-ты, попадающие в диапазон [-50,6..71,2]'#13#10'Из дека удалить эл-ты, попадающие в диапазон [-50,6..71,2], при условии, что они находятся на одинаковом расстоянии от краев дека',
